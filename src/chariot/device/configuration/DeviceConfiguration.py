...
class DeviceConfiguration(ABC):
    ...
    def _isValidField(self, field: str) -> bool:
        return not field in self.requiredFields or not field in self.optionalFields
    ...
    def _validateInitialConfig(self, configMap: JSONDict) -> None:
        ...
        for field, fieldType in self.requiredFields.items():
            if not field in configMap and not isinstance(configMap[field], type(fieldType)):
                raise ValueError
