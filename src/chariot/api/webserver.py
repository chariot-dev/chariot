...
def externalEmit(data: List[JSONObject]) -> None:
    ...
    requests.post(f'http://0.0.0.0:5000{apiBaseUrl}/data/emit', json=data)
...
def modifyNetwork():
    ...
    if locked:
        response = jsonify(
            toDict(
                f'The network cannot be modified because it is being used by {reason}'
            ))
    ...
    if parser.getNewNetworkNameStr() in requestContent:
        ...
        requestContent[TypeStrings.Network_Identifier.value] = requestContent[
            parser.getNewNetworkNameStr()]
...
def deleteNetwork():
    ...
    if locked:
        response = jsonify(
            toDict(
                f'The network cannot be deleted because it is being used by {reason}'
            ))
        response.status_code = 400
        return response
...
def getNetworkDetails():
    ...
    network: Network = NetworkManager.getNetwork(
        networkName).getConfiguration().toDict()
...
def getDeviceDetails():
    ...
    deviceConfig: Configuration = network.getDevice(
        deviceName).getConfiguration()
...
def createDevice():
    ...
    deviceConfig: Configuration = DeviceConfigurationFactory.getInstance(
        payloadConfig)
...
def modifyDevice():
    ...
    if locked:
        response = jsonify(
            toDict(
                f'The device cannot be modified because its network is being used by {reason}'
            ))
        response.status_code = 400
        return response
    ...
    NetworkManager.getNetwork(networkName).getDevice(deviceName).updateConfig(
        requestContent)
...
def deleteDevice():
    ...
    if locked:
        response = jsonify(
            toDict(
                f'The device cannot be deleted because its network is being used by {reason}'
            ))
        response.status_code = 400
        return response
...
def testDBConfiguration():
    ...
    dbConfig: DatabaseConfiguration = DatabaseConfigurationFactory.getInstance(
        requestContent)
...
def modifyDatabaseConfiguration():
    ...
    if locked:
        response = jsonify(
            toDict(
                f'The database configuration cannot be modified because it is being used by {reason}'
            ))
        response.status_code = 400
        return response
    ...
    if parser.getNewDbIdStr() in requestContent:
        ...
        requestContent[TypeStrings.Database_Identifier.value] = requestContent[
            parser.getNewDbIdStr()]
...
def deleteDatabaseConfiguration():
    ...
    if locked:
        response = jsonify(
            toDict(
                f'The database configuration cannot be deleted because it is being used by {reason}'
            ))
        response.status_code = 400
        return response
...
def getDatabaseConfiguration():
    ...
    dbId = parser.getDbNameInURL(request)
    db: DatabaseWriter = DatabaseManager.getDbWriter(
        dbId).getConfiguration().toDict()
...
def createDataCollector():
    ...
    collectionConfig: DataCollectionConfiguration = DataCollectionConfiguration(
        configMap)
...
def getDataCollector():
    ...
    dataCollector: DataCollector = DataCollectionManager.getCollector(
        dataConfigId).getConfiguration().toDict()
...
def deleteDataCollector():
    ...
    if dataConfigId in runningCollectors:
        ...
        response = jsonify(
            toDict(
                f'The data collector with id {dataConfigId} cannot be deleted while it is running.'
            ))
        response.status_code = 400
        return response
...
def startDataCollection():
    ...
    if configId in runningCollectors:
        ...
        response = jsonify(
            toDict(
                f'The data collection with id {configId} is already running.'))
        response.status_code = 400
        return response
...
def endDataCollection():
    ...
    if configId not in runningCollectors:
        ...
        response = jsonify(
            toDict(f'The data collection with id {configId} was not running.'))
        response.status_code = 400
        return response
