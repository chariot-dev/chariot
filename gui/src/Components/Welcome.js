import React from "react";
import { Link } from 'react-router-dom';

import 'bootstrap/dist/css/bootstrap.css';
import '../Welcome.css'

function Welcome() {
  return (
    <div class="container" id="welcomeContent">
      <div>
        <img class="accountAvi" src="data:image/png;base64,"></img>
      </div>
      <div class="accountInfo">
        <text class="font-weight-bold">Name: </text>
        <text id="accountOwner">John Doe</text>
        <div>
        <Link to="/">
          <text id="createAccountLink">View Network Configs</text>
        </Link>
        </div>
        <div>
          <Link to="/">
            <text id="createAccountLink">View IoT Device Configs</text>
          </Link>
        </div>
        <div>
          <Link to="/">
            <text id="createAccountLink">View Data Output Configs</text>
          </Link>
        </div>
        <div>
          <button class="btn btn-primary">Log Out</button>
        </div>
      </div>
      <br></br>
      <br></br>
      <br></br>

      <div id="networkManagerSection">
        <Link to="/">
          <text>Network Manager</text>
        </Link>
        <div>
          <text>View, create, delete, modify, or manage your existing networks.</text>
        </div>
      </div>

      <br></br>

      <div id="dataAnalysisSection">
        <Link to="/">
          <text>Perform Data Analysis</text>
        </Link>
        <div>
          <text>Select a storage unit and perform data analysis.</text>
        </div>
      </div>


    </div>
  );
}

export default Welcome; 
