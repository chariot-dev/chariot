    ...
    fetch(dataCollectionBaseURL, requestOptions)
        ...
        .then(
          ...
          (result) => {
            ...
            else if (result.message) { // If got a message, then failed
              return result;
            }
        })
        .then(
          (res) => {
            if (res) { // Set up state to display error modal upon rerender
              this.setState({ errorMessage: res.message }, () => { // Set the error message
                this.setState({ errorIsOpen: true }); // Then set test error modal to true
              }); 
            }
            else { // Success, so make API to start data collection then send to visualizer
              ...
              .then(
                ...
                (result) => {
                  this.props.history.push({
                    ...
                    runProps: {"Network Name": this.state["Network Name"], "Devices": this.state["Devices"], "configId": this.state.configurationSettings["configId"]}
                  });
                },
                // On error
                (error) => {
  ...
  render() {
    return [
      ...
      <Modal show={this.state.errorIsOpen} key="runError">
        ...
        <Modal.Footer>
          <Button variant="primary" className="float-right" onClick={this.hideErrorModal}>OK</Button>
        </Modal.Footer>
      </Modal>
    ];
  }
