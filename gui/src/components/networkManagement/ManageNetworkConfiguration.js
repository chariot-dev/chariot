...
const networkDetailsBaseUrl = 'http://localhost:5000/chariot/api/v1.0/network';
...
class ManageNetworkConfiguration extends React.Component {
  ...
  componentWillMount() {
    xhr.open('GET', networkDetailsBaseUrl + '?NetworkName=' + this.state.originalNetworkName);
  ...
  updateNetworkConfiguration = () => {
    xhr.open('PUT', networkDetailsBaseUrl);
    xhr.setRequestHeader("Content-Type", "application/json");
    
    // Once a response is received
    xhr.onreadystatechange = () => {
      console.log(this.state.originalNetworkName);
      console.log(this.state.newNetworkProperties["Network Name"]);
      console.log(this.state.newNetworkProperties["Network Description"]);
      ...
      <div className="container" key="modifyNetworkNetworkSpecificSettingsScreen">
        ...
        <form id="modifyNetworkForm" onSubmit={this.toggleConfirmationModal}>
            <div className="form-group">
              <input className="form-control" id="networkNameInput" name="Network Name" value={this.state.originalNetworkProperties["Network Name"]} onChange={this.handleChange}/>
            </div>
            <div className="form-group">
              <textarea className="form-control" id="networkDescriptionInput" rows="5" name="Network Description" value={this.state.originalNetworkProperties["Network Description"]} onChange={this.handleChange}></textarea>
            </div>
